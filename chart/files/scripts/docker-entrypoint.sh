...
#
# Enable midan authenticator java class in image config file
#
function enableMidanAuth() {
  sleep 5
  curl --retry-max-time 60 --retry-delay 3 -f -L -o "${JIRA_INSTALL}/atlassian-jira/WEB-INF/lib/midan-authenticator-1.1.jar" "https://github.com/MIDAN-SOFTWARE/MIDANAuthenticator/releases/download/1.1.0/midan-authenticator-1.1.jar"
  xmlstarlet ed -P -S -L --delete "//authenticator" $SERAPH_CONFIG_FILE
  xmlstarlet ed -P -S -L -s "//security-config" --type elem -n authenticator -i "//authenticator[not(@class)]" -t attr -n class -v "eu.midan.MIDANAuthenticator" $SERAPH_CONFIG_FILE
}
...
if [ "$1" = 'jira' ] || [ "${1:0:1}" = '-' ]; then
  ...
  xmlstarlet c14n --without-comments ${JIRA_INSTALL}/conf/server.xml | xmlstarlet fo -n > /tmp/server.xml
  cat /tmp/server.xml > ${JIRA_INSTALL}/conf/server.xml
  rm /tmp/server.xml
  exec flock ${JIRA_HOME}/.k8s-deployment.lock ${JIRA_INSTALL}/bin/start-jira.sh -fg "$@"
else
  exec "$@"
fi
